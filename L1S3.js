...
const L1S3=({navigation})=> {
  console.log("rendering page")
  const [videos, setVideos] = useState([]);
  
  useEffect(() => {
    const fetchVideos = async () => {
        const querySnapshot =  await firestore().collection('TeachingVideos').where("Level","==",1).where("Title","==","Joint Mobilization").orderBy("Session").get();
