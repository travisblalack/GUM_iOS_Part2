...
const L2S2=({navigation})=> {
  console.log("rendering page")
  const [videos, setVideos] = useState([]);
  
  useEffect(() => {
    const fetchVideos = async () => {
        const querySnapshot =  await firestore().collection('TeachingVideos').where("Level","==",2).where("Title","==","Proper Standing Posture").orderBy("Session").get();
  ...
  try {
    ...
    if (currentUser) {
      ...
      if (userDoc.exists) {
        ...
        navigation.navigate("ModerateList");
